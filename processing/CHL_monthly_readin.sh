...
while [ "$(date -d "$current_date" +%s)" -le "$end_sec" ]; do
    ...
    # Create the expected filename pattern that Copernicus Marine generates
    # The tool creates files with this pattern: cmems_obs-oc_glo_bgc-plankton_my_l4-multi-4km_P1M_CHL-flags_85.06W-59.98W_22.10N-46.02N_YYYY-MM-01.nc
    expected_filename="cmems_obs-oc_glo_bgc-plankton_my_l4-multi-4km_P1M_CHL-flags_85.06W-59.98W_22.10N-46.02N_${month_start}.nc"
    
    # Check if the exact file already exists (without any _(1), _(2) suffixes)
    if [ -f "${output_dir}/${expected_filename}" ]; then
        echo "File already exists for ${year}-${month}: ${expected_filename}. Skipping..."
    else
        # Also check for any files with the same base name but with suffixes like _(1), _(2), etc.
        base_pattern="${output_dir}/cmems_obs-oc_glo_bgc-plankton_my_l4-multi-4km_P1M_CHL-flags_85.06W-59.98W_22.10N-46.02N_${month_start}"
        existing_files=($(ls ${base_pattern}*.nc 2>/dev/null))
