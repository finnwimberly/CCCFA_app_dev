...
def create_combined_ostia_sst_ctd_overview(mean_sst, mean_anom, dates_recent, recent_ssta,
    ...
    ic_lon_min, ic_lon_max = -70.5, -70
    ic_lat_min, ic_lat_max = 41.7, 42.3

    oc_width = oc_lon_max - oc_lon_min
    oc_height = oc_lat_max - oc_lat_min

    ic_width = ic_lon_max - ic_lon_min
    ic_height = ic_lat_max - ic_lat_min

    for ax in [ax1, ax2]:
        oc_patch = Rectangle(
            (ic_lon_min, ic_lat_min),
            ic_width,
            ic_height,
            linewidth=1.5,
            edgecolor="red",
    ...
    ax3.set_title(
        "Inner Cape Region Averaged SST Anomaly (Last 90 Days)",
        fontsize=12,
        weight="bold",
    )
