...
// --- Helper: robust date formatting without timezone drift ---
function formatDateMMDDYYYY(dateStr) {
  // Accepts 'YYYY-MM-DD', 'YYYYMMDD', or full ISO. Returns 'MM/DD/YYYY'.
  ...
  // Handle YYYY-MM-DD (avoid local timezone rollbacks)
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
    const [y,m,d] = dateStr.split('-');
    return `${m}/${d}/${y}`;
  }
  // Fallback for other ISO strings: force UTC display
  try {
...
async function plotCTDMeasurements(profileId, measurements, color) {
  ...
  if (state.selectedProfiles[profileId].date) {
    const date = state.selectedProfiles[profileId].date;
    const formatted = formatDateMMDDYYYY(date) ?? String(date);
    legendName = `${formatted} (${dataType})`;
  } else {
  ...
  await Plotly.addTraces('temp-plot', {
    ...
    line: { shape: 'linear', color: color },
    marker: { color: color },
    hovertemplate: `${legendName}<br>Temp: %{x}<br>Depth: %{y}<extra></extra>`,
    showlegend: false
  });
  ...
  if (dataType === 'CTD' && measurements.salinity && measurements.density) {
    ...
    await Plotly.addTraces('sal-plot', {
      ...
      line: { shape: 'linear', color: color },
      marker: { color: color },
      hovertemplate: `${legendName}<br>Salinity: %{x}<br>Depth: %{y}<extra></extra>`,
      showlegend: false
    });
    ...
    await Plotly.addTraces('dens-plot', {
      ...
      line: { shape: 'linear', color: color },
      marker: { color: color },
      hovertemplate: `${legendName}<br>Density: %{x}<br>Depth: %{y}<extra></extra>`,
      showlegend: false
    });
  }
}
